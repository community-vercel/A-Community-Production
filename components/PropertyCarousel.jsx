...
const PropertyCarousel = ({ properties, serverurl }) => (
    <>
                ...
                <a href={`/property/types/${property.slug}`}>
                  <div className="relative">
                    ...
                    <img
                      src={
                        property.logo
                          ? property.logo.includes('/api/')
                            ? serverurl + property.logo.replace('/api/', '')
                            : serverurl + 'media/' + property.logo
                    ...
                    {property.purpose && (
                      <div
                        className={`absolute top-3 left-3 text-white py-1 px-3 rounded text-xs font-bold ${
                          property.purpose === 'rent' ? 'bg-yellow-500' : 'bg-red-600'
                        }`}
                      >
                  ...
                  <div className="p-4">
                    {/* Price */}
                    <h6 className="font-bold text-lg mb-1 text-gray-900">$ {property.price}</h6>
                    ...
                    <p className="text-blue-600 font-medium mb-2">
                      {property.area} {property.unit ? property.unit.toUpperCase() : ''}
                    </p>
                    {/* Rooms & Baths Icons */}
                    <div className="flex space-x-4">
                      <span className="text-sm flex items-center">🛏 {property.bed}</span>
                      <span className="text-sm flex items-center">🛁 {property.baths}</span>
                    </div>
                  </div>
                </a>
              </div>
