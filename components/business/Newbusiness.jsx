...
export default function Newbusiness({initialbusinessData}) {
  ...
  const logoUrl = business?business.logo
...
const favoritePageHide = null ;
  ...
  useEffect(() => {
    const fetchReplies = async (reviewId) => {
      const response = await fetch(`${serverurl}get_replies?review_id=${reviewId}`);
  ...
  const hasSocials = business.socials && Object.values(business.socials).some(url => url);
  ...
  const constructBreadcrumbItems = () => {
    ...
    const currentCategory = business && business.slug === params.slug ? business : null;
    console.log("Currenet category",currentCategory)
    if (currentCategory) {
      breadcrumbItems.push({ label: currentCategory.name, href: `/business/categories//${currentCategory.slug}` }); // Link to the current category
  ...
  const metadata =  initialbusinessData ? {
    ...
    openGraph: {
      title:  initialbusinessData.metaname,
      description:  initialbusinessData.metades || `Explore ${ initialbusinessData.description} for excellent services and offerings.`,
      url: `${frontend}business/categories/${ initialbusinessData.slug || "default-slug"}`,
      images: [
        `${serverurl}${initialbusinessData.logo?.includes('/api/media/') 
    ...
    twitter: {
      ...
      title:  initialbusinessData.metaname || `${ initialbusinessData.name} - Explore Our Services`,
      description:  initialbusinessData.metades || `Find out more about ${ initialbusinessData.name}, a leader in the  ${ initialbusinessData.categories &&  initialbusinessData.categories[0].category__name || " initialbusinessData"} category.`,
      images: [
        `${serverurl}${initialbusinessData.logo?.includes('/api/media/') 
          ? initialbusinessData.logo.replace('/api/media/', 'media/') 
          : initialbusinessData.logo || "/path/to/default_logo.jpg"}`
    ...
    description: "Discover top businesses, services, and products across various industries.",
    keywords: "businesses, services, products, top-rated companies, best businesses",
    openGraph: {
      title: "Explore Top Businesses and Services",
      description: "Find trusted businesses in your area for a variety of services and products.",
    ...
    twitter: {
      card: "summary_large_image",
      title: "Explore Top Businesses and Services",
      description: "Connect with reliable businesses across multiple categories.",
      images: ["https://yourwebsite.com/path/to/default_image.jpg"],
    },
  };
          ...
          <div className="relative">
            ...
            <ListTopBanner2
             img={business.images && business.images.length > 0 
              ...
              label={business && business.categories.length && business.categories[0].category__name}
...
{business.location || business.city || business.state || business.zip 
  ? [
...
{business.website ? business.website : "No email provided"}
</Link>
</span>
</div>
)}
...
<Image src={twitter} alt="" />
  ...
  <div>
    {business.discount_code && business.discount_message ? (
      <span>
        {business.discount_message.split("[code]").map((part, index) => (
  ...
  <div className="flex flex-col space-y-4">
    {business.categories
      .filter((item, index, self) =>
        index === self.findIndex((t) => t.category__id === item.category__id)
      )
      .map((category) => (
        <div
        ...
        >
          <H3 className="text-lg font-semibold text-blue-600">
            <Link href={`/places/category/${category.category__slug}`}>
          ...
          <div className="flex flex-wrap items-center ml-4">
            {business.categories
              .filter((subItem) => subItem.subcategory__id && subItem.category__id === category.category__id)
              .map((subItem, index) => (
                <Link
                ...
                >
                  {index > 0 && <span className="text-gray-500 mx-1">|</span>} {/* Separator */}
        ...
        {user.id && user.id!==business.user_id ? (
          ...
          <div className="flex justify-between flex-wrap gap-4 items-center">
            <span className="text-bold">
You Cant Review Your Own Business            </span>
    ...
    <div className="mt-2">
      {reviews.map((review) => (
        <div className="py-3 mb-3 border-b-2" key={review.id}>
          ...
          {review.review_files && review.review_files.split(",").length > 0 && (
          ...
          {review.owner_reply && (
            <div className="bg-white p-6 rounded-lg shadow-md mt-4">
              <H4 className="text-lg font-semibold text-gray-800">Owner Reply</H4>
              <p className="mt-3 text-gray-700">{review.owner_reply}</p>
          ...
          {/* Display the replies */}
          <div className="mt-2">
            <H3  className="text-xl font-semibold">Review Replies</H3 >
            {replies[review.id] && replies[review.id].length > 0 ? (
              <div className="mt-4">
                {(expandedReplies[review.id] ? replies[review.id] : replies[review.id].slice(0, 2)).map((reply) => (
                  <div key={reply.id} className="bg-white p-5 rounded-lg shadow-lg mt-2 border border-gray-200 transition-shadow hover:shadow-xl">
                      ...
                      <div className="ml-4 w-full">
                        <div className="flex justify-between items-center">
                          <p className="text-gray-800 font-medium text-sm">
                            {reply.isreviewwown ? reply.user__name + ' (Owner)' : reply.user__name}
                          </p>
                          <span className="text-gray-500 text-xs">{new Date(reply.created_at).toLocaleString()}</span>
                        </div>
                        <p className="mt-2 text-gray-400 text-base">{reply.content}</p>
                      </div>
                    </div>
                  </div>
                ))}
                ...
                {replies[review.id].length > 2 && (
                  ...
                  >
                    {expandedReplies[review.id] ? 'Show Less' : 'Read More'}
                  </button>
                )}
              </div>
            ) : (
              <p className="text-sm text-gray-500 mt-2">No replies for this review.</p>
            )}
          </div>
          {user.role && (
    <div className="mt-4">
      ...
      >
        {visibleReplyInput === review.id ? "Cancel" : "Reply"}
      </button>
