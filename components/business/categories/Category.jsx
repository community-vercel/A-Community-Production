...
const CategoryPage = ({ initialCategoryData }) => {
  console.log("New Datasssssssssssssssssss", initialCategoryData);
  ...
  console.log("Is SuCategory", isSubcategory);
  const params = useParams();
  const searchParams = useSearchParams();
  const { user_meta, user } = useSelector((state) => state.auth);
  const myselectedLanguage = user_meta.selectedLanguage || "";
  const countryid = 233;
  ...
  }, [params.slug, myselectedLanguage]);
  ...
  const fetchMoreBusiness = async () => {
    ...
    console.log("Fetching more businesses...");
  ...
  const metadata = {
    title: initialCategoryData.category?.metaname?.[0] || initialCategoryData.category?.category_name?.[0] || "Default Title",
    description: initialCategoryData.category?.metades?.[0] || initialCategoryData.category?.description || "Default Description",
    keywords: initialCategoryData.category?.keyword || "default, keywords",
    openGraph: {
      title: initialCategoryData.category?.metaname?.[0] || initialCategoryData.category?.category_name?.[0] || "Default OG Title",
      description: initialCategoryData.category?.metades?.[0] || initialCategoryData.category?.description || "Default OG Description",
    ...
    twitter: {
      ...
      title: initialCategoryData.category?.metaname?.[0] || initialCategoryData.category?.category_name?.[0] || "Default Twitter Title",
      description: initialCategoryData.category?.metades?.[0] || initialCategoryData.category?.description || "Default Twitter Description",
      images: [
        initialCategoryData.category?.cover?.includes('/api/media/')
          ? `${serverurl}/media/${initialCategoryData.category.cover.replace('/api/media/', '')}`
