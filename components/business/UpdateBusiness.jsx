  ...
  const handleSelectChange = (option) => {
    ...
    const toggleSubcategory = (subcategory, parent) => {
      if (alreadySelected) {
        ...
        newSelectedItems = newSelectedItems.filter(item => item.value !== subcategory.value);
  
        // If no subcategories remain selected, remove the parent
        const remainingSubsSelected = parent.subcategories.some(sub => isSelected(sub));
        if (!remainingSubsSelected) {
          newSelectedItems = newSelectedItems.filter(item => item.value !== parent.value);
        }
      } else {
        ...
        const allSubsSelected = parent.subcategories.every(sub => isSelected(sub));
...
const flattenSelectedItems = (items) => items.flatMap(item => [item, ...(item.subcategories || [])]);
  ...
  const onSubmit = async (formData) => {
    ...
    try {
      ...
      if (images) {
        ...
        imagesDB.split(",").forEach((imagePath) => {
...
selectedData.forEach((data) => {
  ...
  const breadcrumbItems = [
...

    {
      ...
      href: "/business/update/" + params.id,
    },
  ];
  return (
    <>
