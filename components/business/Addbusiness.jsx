...
const Addbusiness = () => {
  ...
  const handleSelectChange = (option) => {
    ...
    if (option.isParent) {
      // Handle parent selection/deselection
      if (alreadySelected) {
        // Deselect parent and all its subcategories
        newSelectedItems = newSelectedItems.filter(
          (item) => item.value !== option.value && item.parentId !== option.value
        );
      } else {
        // Select parent and all its subcategories if not already selected
        newSelectedItems.push(
          option,
          ...option.subcategories.filter((sub) => !isSelected(sub))
        );
      }
    } else {
      ...
      if (alreadySelected) {
        ...
        // Check if all subcategories are still selected and parent exists
        if (parent) {
          const allSubcategoriesSelected = parent.subcategories.every((sub) =>
            newSelectedItems.some((item) => item.value === sub.value)
          );
  
          // If not all subcategories are selected, remove the parent
        ...
        // Select subcategory and check if all subcategories are now selected
        newSelectedItems.push(option);
  
        // Ensure parent exists before checking its subcategories
        if (parent) {
          const allSubcategoriesSelected = parent.subcategories.every((sub) =>
            newSelectedItems.some((item) => item.value === sub.value)
          );
          ...
          if (allSubcategoriesSelected && !isSelected(parent)) {
...
selectedItems.forEach((item) => {
    if (item.isParent) {
        ...
        if (parentCategory) {
            selectedData.push({ categoryId: parentCategory.value, subcategoryId: item.value });

            // Ensure the main category is also included
            if (!selectedValues.has(parentCategory.value)) {
                selectedData.push({ categoryId: parentCategory.value, subcategoryId: null });
            }
        }
    }
});
...
selectedData.forEach((data) => {
    if (data.categoryId !== null) {  // Ensure categoryId is defined
